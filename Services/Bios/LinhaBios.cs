internal class LinhaBios : BiosTweakerBase
{
    ...
    public byte ImageChecksum
    {
        get
        {
            return base.buffer[base.offset + this.RomHeader.E00B.PE004 - 1];
        }
        set
        {
            base.buffer[base.offset + this.RomHeader.E00B.PE004 - 1] = value;
        }
    }
    ...
    {
        ulong num = 0UL;
        for (int index = base.offset; index < base.offset + this.RomHeader.E00B.PE004 - 1; ++index)
            num += base.buffer[index];
        return (byte)(byte.MaxValue - (ulong)((long)num - 1L & byte.MaxValue));
    }

    private bool E003(int param0)
    {
        if (param0 != -1 && param0 > base.offset)
            return param0 < base.offset + this.RomHeader.E00B.PE004;
        return false;
    }
}
